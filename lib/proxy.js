...
USSDSocket.prototype = {
  ...
  onOpen: function(err){
    ...
    this.connected=true;
    this.modem.getUSSD("#124#",this.onSend.bind(this));
  },

  send: function(data){
    var session = new Session();
    session.callback = this.onSend;
    session.parseResponse = this.parseResponse; 
    session.modem=this.modem;
    session.execute = function() {
        //this.query('##', session.parseResponse);
    }
    session.start();
  },
